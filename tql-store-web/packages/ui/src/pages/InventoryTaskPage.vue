  ...
  <div class="cost-page">
    ...
    <a-card class="cost-panel">
      <div class="panel-toolbar">
        <a-space><span>当前门店</span><a-select v-model="storeId" placeholder="请选择门店" class="store-select" @change="load"><a-option v-for="store in stores" :key="store.id" :value="store.id">{{ store.name }}</a-option></a-select></a-space>
        <a-input-search v-model="keyword" allow-clear placeholder="搜索任务名称或编号" class="toolbar-search" />
      </div>
    ...
    <a-modal v-model:visible="visible" title="新建盘点任务" :ok-loading="saving" @ok="save">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="盘点时间" required><a-range-picker v-model="form.range" show-time value-format="YYYY-MM-DDTHH:mm:ss" class="full-width-control" /></a-form-item>
    ...
    <a-drawer v-model:visible="detailVisible" title="盘点明细与差异" :width="760" :footer="false">
      ...
      <a-table class="detail-table" :data="detailItems" :columns="detailColumns" row-key="snapshotId" :pagination="false">
...
const stores=ref<StoreOption[]>([]),storeId=ref<number>(),tasks=ref<InventoryTask[]>([]),keyword=ref(''),loading=ref(false),visible=ref(false),saving=ref(false);
...
function statusInfo(s:string){const statusMap:Record<string,{label:string;color:string}>={DRAFT:{label:'草稿',color:'gray'},IN_PROGRESS:{label:'盘点中',color:'blue'},PENDING_REVIEW:{label:'待审核',color:'orange'},APPROVED:{label:'已审核',color:'green'},CLOSED:{label:'已关账',color:'purple'}};return statusMap[s]||{label:s,color:'gray'}}
function formatTime(value:string){return value?value.replace('T',' ').slice(0,16):'—'}
</script>
<style scoped>.store-select{width:240px}.toolbar-search{width:260px}.full-width-control{width:100%}.detail-table{margin-top:var(--tql-space-4)}.diff-up{color:var(--tql-danger);font-weight:600}.diff-down{color:var(--tql-success);font-weight:600}</style>
