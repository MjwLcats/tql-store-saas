<template>
  <div class="cost-page">
    ...
    <a-card class="cost-panel">
      <div class="panel-toolbar"><a-space><span>当前门店</span><a-select v-model="storeId" style="width:240px" @change="load"><a-option v-for="store in stores" :key="store.id" :value="store.id">{{store.name || store.storeName}}</a-option></a-select></a-space><a-input-search v-model="keyword" allow-clear placeholder="搜索菜品或状态" style="width:260px"/></div>
    ...
    <a-modal v-model:visible="syncVisible" title="请选择菜品" width="860px" :footer="false" unmount-on-close>
      <a-form :model="syncQuery" layout="inline" class="sync-filter">
        <a-form-item required><a-select v-model="syncQuery.shopId" allow-search placeholder="请选择门店" style="width:210px" @change="searchSyncFoods"><a-option v-for="shop in syncSourceShops" :key="shop.relateid" :value="shop.relateid">{{shop.deptName}}</a-option></a-select></a-form-item>
        <a-form-item><a-input v-model="syncQuery.foodName" allow-clear placeholder="请输入菜品名称" style="width:180px" @press-enter="searchSyncFoods"/></a-form-item>
        <a-form-item><a-input v-model="syncQuery.foodCode" allow-clear placeholder="请输入菜品编码" style="width:180px" @press-enter="searchSyncFoods"/></a-form-item>
      ...
      <a-table v-model:selected-keys="syncSelectedKeys" :columns="syncColumns" :data="syncRows" row-key="foodID" :loading="syncLoading" :row-selection="{type:'checkbox',showCheckedAll:true}" :pagination="false">
        <template #price="{record}"><span v-if="record.foodPrices?.length">{{record.foodPrices.map((item:any)=>`${item.foodPrice} / ${item.unit}`).join('、')}}</span><span v-else>-</span></template>
...
const stores=ref<any[]>([]),storeId=ref<number>(),records=ref<CostBom[]>([]),dishes=ref<CostDish[]>([]),materials=ref<CostMaterial[]>([]),units=ref<CostUnit[]>([]),loading=ref(false),saving=ref(false),visible=ref(false),keyword=ref('');
...
function dishName(id:number){return dishes.value.find(d=>d.id===id)?.dishName||`菜品 ${id}`}function formatTime(v:string){return v?v.replace('T',' ').slice(0,16):'—'}function statusInfo(s:string){return({DRAFT:{label:'草稿',color:'gray'},PENDING:{label:'待审核',color:'orange'},PUBLISHED:{label:'已发布',color:'green'},REJECTED:{label:'已驳回',color:'red'},DISABLED:{label:'已停用',color:'gray'}} as any)[s]||{label:s,color:'gray'}}
</script>
<style scoped>.bom-items{width:100%;padding:14px;border:1px solid #e5e6eb;border-radius:6px;background:#f7f8fa}.bom-item-row{display:flex;gap:10px;margin-bottom:10px}.sync-filter{margin-bottom:4px}.sync-actions{margin:6px 0 16px}.sync-safe-tip{margin-left:8px;color:#86909c;font-size:12px}.sync-pagination{display:flex;align-items:center;justify-content:space-between;margin-top:18px;color:#86909c}</style>
